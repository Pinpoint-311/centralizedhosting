...
/**
 * The "who provides which API key" matrix. State-provided keys are brokered by
 * the panel and injected at provision time; town-owned keys are entered by the
 * town in its own instance. Infrastructure keys are always state-owned (locked).
 */
  ...
  return (
    <div className="space-y-3">
      {catalog.assignable.map((svc) => {
        ...
        const isState = owner === 'state'
        return (
          <div
            key={svc.id}
            className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4 rounded-xl bg-white/[0.03] border border-white/10"
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-medium text-white">{svc.label}</span>
                <code className="text-[11px] text-white/40 bg-white/5 rounded px-1.5 py-0.5 truncate">
                  {svc.keys.join(', ')}
                </code>
              </div>
              <p className="text-sm text-white/50 mt-0.5">{svc.description}</p>
              {isState && <p className="text-xs text-indigo-300/70 mt-1">{svc.state_hint}</p>}
            </div>
            ...
            <div className="inline-flex rounded-xl bg-white/5 border border-white/10 p-1 shrink-0">
              ...
              >
                <Landmark className="w-3.5 h-3.5" /> State provides
              </button>
              <button
                type="button"
                disabled={disabled}
              ...
              >
                <Building2 className="w-3.5 h-3.5" /> Town provides
              </button>
            </div>
          </div>
        )
      ...
      <div className="p-4 rounded-xl bg-white/[0.02] border border-white/10 border-dashed">
        ...
        <p className="text-xs text-white/40 mb-3">
          These are provisioned automatically and never editable by the town: database
          credentials, encryption keys, backups, and the app secret key.
        </p>
